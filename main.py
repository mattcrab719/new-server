...
class MyBot(commands.Bot):
    ...
    def save_score(self, user_id):
        ...
        with open('scores.json', 'w') as f: json.dump(self.scores, f)
...
async def update_lb():
    ...
    # Clears old messages and posts new one
...
async def start_rating(interaction, img1, img2, img3, gender, role_map):
    ...
    class RatingView(ui.View):
        ...
        async def handle_click(self, inter, name):
            self.results[name] += 1
    ...
    for name in role_map.keys():
        ...
        async def create_callback(n):
            return lambda i: view.handle_click(i, n)
        btn.callback = await create_callback(name)
        view.add_item(btn)

    embed = discord.Embed(title=f"{gender} Rating Request", description=f"Submitter: {interaction.user.mention}")
    ...
    role = interaction.guild.get_role(role_map[top_role_name])
